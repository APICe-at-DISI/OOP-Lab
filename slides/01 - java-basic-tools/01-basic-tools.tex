...
\fr{Java Development Kit (JDK)}{
	...
	\bl{Esistono diverse distribuzioni di Java}{
		\begin{description}
			...
			\item[Java EE] -- Java Enterprise Edition: JDK + set esteso di librerie
		\end{description}
	}
...
\fr{Java Development Kit (JDK)}{
    ...
    \begin{block}{Riferimenti per il corso}
        \begin{itemize}
            ...
            \end{itemize}
            \item Chi lo desidera può sperimentare con le versioni più recenti (fino a Java 15)
        \end{itemize}
    \end{block}
}
...
\fr{Sistemi Windows}{
    \bl{}{
        ...
        \iz{
            ...
            \begin{itemize}
                \item Può essere un modo ragionevole di avere bash in ambiente Windows
            \end{itemize}
        }
    }
}
...
\fr{Preparazione Ambiente di Lavoro}{
	\iz{
        ...
        \item Si comprenda in qualche cartella si è usando l'apposito comando
        \iz{
            \item Generalmente il prompt si apre nella directory principale dell'utente
            \item Nel caso in cui si usasse zsh dentro Docker, il terminale parte dalla cartella \texttt{DockerHome} sul desktop, e non è in grado di uscirne:
            È quindi necessario spostare i file decompressi dentro quest'ultima.
        }
